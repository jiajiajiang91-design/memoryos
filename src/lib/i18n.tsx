...
const DICT: Record<string, Entry> = {
  ...
  "fileViewer.overBudget": {
    zh: "记忆卡片已 {n} 字，超过 1200 字，AI 开场要读的内容会太长。建议整理或精简，不影响保存。",
    en: "Memory Cards at {n} chars, over the 1200 limit — the AI's opening read gets too long. Consider tidying; saving still works.",
  },
  ...
  "dashboard.injectionTitle": { zh: "AI 开场读什么", en: "What the AI reads first" },
  "dashboard.injectionCards": { zh: "记忆卡片", en: "Memory Cards" },
  "dashboard.injectionEntries": { zh: "记忆库", en: "Memory Library" },
  "dashboard.injectionHint": {
    zh: "复制开始对话指令和 AI 连接都从这里取记忆。记忆库按重要程度自动挑选，不超过 1200 字",
    en: "Both the start prompt and connected AIs read from this source. Library picks by importance within 1200 chars",
  },
  "entryLib.injectionToggle": { zh: "AI 开场读记忆库", en: "AI reads the library first" },
  "entryLib.injectionOnToast": { zh: "已切换：AI 开场改读记忆库挑选的内容", en: "Switched: the AI now opens with the library's selection" },
  "entryLib.injectionOffToast": { zh: "已切回：AI 开场读记忆卡片", en: "Switched back: the AI opens with Memory Cards" },
  "entryLib.aiViewHintLive": {
    zh: "开关已开：点复制开始对话指令时，AI 拿到的就是这段按重要程度挑选的文字。随时可关回记忆卡片。",
    en: "Switch is on: the start prompt now carries exactly this importance-ranked text. Turn it off anytime to go back to Memory Cards.",
  },
  "entryLib.aiViewHint": {
  ...
  "entryLib.migrateGlobalCta": { zh: "把关于我整理进全局库", en: "Convert About Me into the global library" },
  "entryLib.skillEmptyHint": {
    zh: "技能库放做事的方法、模板和避坑经验，每个项目都能用上，AI 开场会连同项目记忆一起读到",
    en: "The skill library holds how-to knowledge, templates and lessons learned. It applies to every project and is injected alongside project memory",
  },
  "entryLib.skillExamples": {
    zh: "比如固定的工作流程、常用的提示词写法、某个工具的使用心得",
    en: "For example a fixed workflow, a prompt pattern you reuse, or hard-won knowledge about a tool",
  },
  "entryLib.addEntry": { zh: "手写一条", en: "Add entry" },
  "entryLib.addPlaceholder": { zh: "一句话写下一条记忆", en: "Write one memory in a sentence" },
  "entryLib.addApply": { zh: "添加", en: "Add" },
  "entryLib.addDone": { zh: "已添加进当前库", en: "Added to this library" },
  "entryLib.collectSkills": { zh: "汇集技能条目", en: "Collect skill entries" },
  "entryLib.collectSkillsHint": {
    zh: "把项目库和全局库里标了技能类型的条目移进技能库",
    en: "Move entries tagged as skill from project and global libraries into here",
  },
  "entryLib.collectDone": { zh: "已汇集 {n} 条进技能库", en: "Collected {n} entries" },
  "entryLib.collectNone": { zh: "各库里没有标了技能类型的条目", en: "No skill-tagged entries found in other libraries" },
  "entryLib.autoRelate": { zh: "一键找关联", en: "Find links" },
  "entryLib.autoRelateHint": { zh: "找出内容相近的记忆，你确认后才建关联。更准的整理用复制给 AI 整理", en: "Finds memories with similar text — links are only created after you confirm. For precision, use the AI tidy-up" },
  "entryLib.autoRelateProposed": { zh: "找到 {n} 组相近记忆，等你确认", en: "Found {n} pairs awaiting your confirmation" },
  "entryLib.autoRelateNone": { zh: "没有找到新的相近记忆", en: "No new similar memories found" },
  "entryLib.autoRelatePendingOnly": { zh: "没有新发现，还有 {n} 条提案等你确认", en: "Nothing new — {n} proposals still awaiting confirmation" },
  "entryLib.relPending": { zh: "关联提案 {n} 条待确认", en: "{n} link proposals pending" },
  "entryLib.relPendingHint": { zh: "接受才建关联，不要的以后不再提", en: "Accept to link; declined pairs won't come back" },
  "entryLib.relAccept": { zh: "接受", en: "Accept" },
  "entryLib.relReject": { zh: "不要", en: "Decline" },
  "entryLib.relAcceptAll": { zh: "全部接受", en: "Accept all" },
  "entryLib.relAcceptedAll": { zh: "已建立 {n} 条关联", en: "Created {n} links" },
  "entryLib.findDup": { zh: "找重复", en: "Find duplicates" },
  "entryLib.findDupHint": { zh: "找出内容基本重复的记忆，确认后合并成一条，被并的收进档案不删", en: "Finds near-duplicate memories. Confirm to merge; the absorbed one is archived, not deleted" },
  "entryLib.dupProposed": { zh: "找到 {n} 组疑似重复，等你确认", en: "Found {n} likely duplicates awaiting confirmation" },
  "entryLib.dupNone": { zh: "没有发现重复的记忆", en: "No duplicates found" },
  "entryLib.dupPendingOnly": { zh: "没有新发现，还有 {n} 组疑似重复等你确认", en: "Nothing new — {n} duplicate pairs still awaiting confirmation" },
  "entryLib.dupPending": { zh: "疑似重复 {n} 组待确认", en: "{n} duplicate pairs pending" },
  "entryLib.dupPendingHint": { zh: "合并后保留一条，被并的收进档案；关联和重要度都并过去，不丢信息", en: "Merging keeps one and archives the other; links and importance carry over" },
  "entryLib.dupKeep": { zh: "保留", en: "Keep" },
  "entryLib.dupDrop": { zh: "并入后归档", en: "Absorb & archive" },
  "entryLib.dupMerge": { zh: "合并", en: "Merge" },
  "entryLib.dupNotDup": { zh: "不是重复", en: "Not a duplicate" },
  "entryLib.merged": { zh: "已合并，被并的一条在已归档区可查", en: "Merged — the absorbed entry is in the archive" },
  "entryLib.importProposals": { zh: "另有 {n} 条 AI 调整建议等你确认", en: "{n} AI adjustment proposals awaiting confirmation" },
  "entryLib.archPending": { zh: "AI 建议归档 {n} 条待确认", en: "{n} AI archive suggestions pending" },
  "entryLib.archPendingHint": { zh: "确认才收进档案，随时可捞回；不要的以后不再提", en: "Confirm to archive (reclaim anytime); declined ones won't come back" },
  "review.entriesTarget": { zh: "记忆库", en: "Memory library" },
  "entryLib.miscOutletHint": { zh: "零散里已经攒了 {n} 条。点复制给 AI 整理让 AI 归类，改完从导回 md 贴回来", en: "{n} entries piled up in Misc. Use Copy for AI tidy-up to classify them, then paste the result back via Import md" },
  "entryLib.migrateDone": { zh: "已整理 {n} 条记忆进记忆库", en: "Converted {n} entries" },
  ...
  "entryLib.relHint": { zh: "这条记忆和哪条有关系。相关的记忆会优先一起给 AI", en: "Link this memory to another. Related memories are given to the AI together" },
  ...
  "entryLib.suggestArchiveHint": { zh: "重要度低、30 天没动过。点击收进档案，随时可捞回；钉住的不受影响", en: "Low importance and idle for 30 days. Click to archive — reclaim anytime; pinned entries are unaffected" },
  ...
  "entryLib.reclaim": { zh: "捞回", en: "Reclaim" },
  "entryLib.reasonSuperseded": { zh: "被替代", en: "Superseded" },
  "entryLib.reasonLowWeight": { zh: "重要度低", en: "Low importance" },
  "entryLib.reasonManual": { zh: "手动归档", en: "Manually archived" },
  "entryLib.tierHigh": { zh: "高", en: "High" },
  "entryLib.tierMid": { zh: "中", en: "Mid" },
  "entryLib.tierLow": { zh: "低", en: "Low" },
  "entryLib.tierHint": { zh: "重要度，点击切换高中低", en: "Importance — click to change" },
  "entryLib.pinHint": { zh: "钉住：不会被自动降低重要度或归档", en: "Pin: never auto-demoted or archived" },
  "entryLib.searchPlaceholder": { zh: "搜记忆内容或编号", en: "Search text or id" },
  "entryLib.filterAll": { zh: "全部", en: "All" },
  "entryLib.filterKind": { zh: "类型", en: "Kind" },
  "entryLib.filterSource": { zh: "来源", en: "Source" },
  "entryLib.filterTier": { zh: "重要度", en: "Importance" },
  ...
  "entryLib.graphHint": { zh: "相连的记忆聚成一团。滚轮缩放，拖空白平移，拖动点重新摆放", en: "Linked memories cluster together. Scroll to zoom, drag the background to pan, drag a dot to move it" },
  "entryLib.graphReset": { zh: "复位", en: "Reset" },
  "entryLib.graphHoverIdle": { zh: "悬停一个点，完整内容显示在这里", en: "Hover a dot to read its full text here" },
  ...
  "entryLib.legendSupersedes": { zh: "取代", en: "Supersedes" },
  "entryLib.legendSize": { zh: "点越大关联越多", en: "Bigger dot, more links" },
  "entryLib.legendColor": { zh: "颜色越深越重要", en: "Darker means more important" },
  ...
  "entryLib.truthVerified": { zh: "已核实", en: "Verified" },
  "entryLib.truthUnverified": { zh: "未核实", en: "Unverified" },
  ...
  "entryLib.exported": { zh: "已复制到剪贴板，可以发给别的 AI 或粘到文件里手改", en: "Copied — send it to another AI or edit it by hand" },
  ...
  "help.step3Desc": {
    zh: "AI 会按固定格式输出对话总结：做了什么、你确认的决定（带原话）、AI 的建议（待确认）、下次起点；项目有记忆卡片时，还会附上整理好的卡片新版。",
    en: "The AI outputs a structured handoff: what happened, decisions you confirmed (with quotes), AI suggestions (pending), next starting point — plus an updated version of your Memory Cards.",
  },
