...
const DICT: Record<string, Entry> = {
  ...
  "dashboard.injectionHint": {
    zh: "复制开场提示词和 AI 连接都从这里取记忆。记忆库按重要程度自动挑选，不超过 1200 字",
  ...
  "entryLib.autoRelateHint": { zh: "找出内容相近的记忆变成提案，你确认后才建关联。机械初版，更准的整理用复制给 AI 整理", en: "Finds memories with similar text as proposals — links are only created after you confirm. Rough first pass; use the AI tidy-up for precision" },
  ...
  "entryLib.dupPendingHint": { zh: "合并后保留一条，被并的收进档案；关联和档位都并过去，不丢信息", en: "Merging keeps one and archives the other; links and weight carry over" },
  ...
  "review.entriesTarget": { zh: "记忆条目库", en: "Memory entries" },
  "entryLib.miscOutletHint": { zh: "零散是暂放不是归宿，已经攒了 {n} 条。用复制给 AI 整理让 AI 给它们归类，改完导回就各回各家", en: "Misc is a holding area, not a home — {n} entries piled up. Use the AI tidy-up to classify them, then import the result" },
  ...
  "entryLib.reasonLowWeight": { zh: "权重过低", en: "Low weight" },
  ...
  "entryLib.legendColor": { zh: "颜色深浅是权重高中低", en: "Color depth is weight tier" },
